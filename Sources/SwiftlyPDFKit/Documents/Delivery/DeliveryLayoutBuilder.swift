...
// MARK: - DeliveryLayoutBuilder (internal)

enum DeliveryLayoutBuilder {

    // MARK: - Standard

    static func standardLayout(
        invoice: InvoiceDocument,
        supplement: DeliverySupplement,
        theme: InvoiceTheme,
        pageSize: PageSize
    ) -> PDF {
        ...
        let page1Overhead: CGFloat = headerHeight + 12 + shipToH + 20 + metaHeight + 20
        ...
        // Include a "Notes" column only when at least one line has a detail string.
        let hasNotes = invoice.lines.contains { $0.detail != nil }

        func deliveryTable(rows: [InvoiceLine]) -> some PDFContent {
        ...
        for (pageIdx, chunk) in chunks.enumerated() {
            ...
            let page = Page(size: pageSize, margins: margins) {
                if isFirst {
                    ...
                    deliveryMetaSection(invoice: invoice, theme: theme, fmt: fmt)
                    Spacer(height: 20)
                }

                deliveryTable(rows: chunk)
    ...
    static func deliveryMetaSection(
        invoice: InvoiceDocument,
        theme: InvoiceTheme,
